...
async def test_existing_user_gets_active_membership(mock_org_svc):
    """Known user who isn't a member → active membership, no Cognito call."""
    user = _mock_user()
    new_membership = _mock_membership()

    mock_org_svc.find_user_by_email = AsyncMock(return_value=user)
...
@patch("app.services.invite_service.org_service")
async def test_active_member_raises_already_exists(mock_org_svc):
...
@patch("app.services.invite_service.org_service")
async def test_pending_invite_raises_already_exists(mock_org_svc):
...
async def test_unknown_email_creates_cognito_and_placeholder(
    ...
    mock_org_svc.find_user_by_email = AsyncMock(return_value=None)
    mock_org_svc.create_placeholder_user = AsyncMock(return_value=placeholder)
    mock_org_svc.create_membership = AsyncMock(return_value=new_membership)
    ...
    mock_org_svc.create_placeholder_user.assert_called_once_with(db, "new@test.com")
    mock_org_svc.create_membership.assert_called_once_with(
        db=db,
        org_id=org_id,
...
@patch("app.services.invite_service.org_service")
async def test_is_new_org_skips_duplicate_check(mock_org_svc):
